...
public class UserDefaultsItem<UserDefaultType> {
  ...
  private(set) var type: UserDefaultProxy!
  let key: String

  public required init(_ key: String) {
    self.key = key
    if let type = _configureType() {
      self.type = type
    } else {
      fatalError("define type not support in NSUserDefaults")
    }
  }

  public var wrappedValue: UserDefaultType? {
    get {
      return self[key]
    }
    set {
  ...
  private func _configureType() -> UserDefaultProxy? {
    let type = T.self
    if type == [String : AnyObject].self ||
      ...
      return .bool
    } else if type == Any.self || type == AnyObject.self {
      return .object
    } else {
      return nil
    }
...
extension UserDefaultsItem : CustomStringConvertible, CustomDebugStringConvertible {
